...
try {
    $stmt = $conexao->prepare("DELETE FROM PARCERIA WHERE id_parceria = ?");
    ...
    error_log('Parceria EXCLUIR error: ' . $e->getMessage());
    $retorno['mensagem'] = 'ERRO: Não foi possível excluir. A parceria pode estar ligada a uma Avaliação. (' . $e->getMessage() . ')';
    echo json_encode($retorno);
    exit;
}
?>