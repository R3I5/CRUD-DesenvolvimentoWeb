...
try {
    ...
    if($stmt->execute()){
        if($stmt->affected_rows > 0){
            ...
            $retorno['mensagem'] = 'Usuário não encontrado ou já removido.';
        }
    } else {
    ...
    error_log('Usuario EXCLUIR error: ' . $e->getMessage());
    $retorno['mensagem'] = 'ERRO: Não foi possível excluir. O usuário pode estar ligado a um Espaço ou Parceria. (' . $e->getMessage() . ')';
    echo json_encode($retorno);
    exit;
}
?>